    ...
    for i in dict_of_OCR.keys():
        ...
        if number_cols[i] < max_col:
            for key in dict_of_rows.keys():
                ...
                if abs(yck - yc) < offset_x and abs(hk - h) < offset_x:
                    ...
                    # if "Items" in dict_of_OCR[key] and len(dict_of_rows[key]) > 1:
                    #     label = "Item"
                    # elif "Amount" in dict_of_OCR[key] and len(dict_of_rows[key]) > 1:
                    #     label = "Price"
                    if "Grand total" in dict_of_OCR[key] and len(dict_of_rows[key]) > 2:
                        label = "Total price"
            if new_col:
                dict_of_rows[i] = []
        ...
        for key in dict_of_cols.keys():
            ...
            if abs(xck - xc) < offset_x and abs(wk - w) < offset_x and number_cols[i] == number_cols[key] :
                ...
                # key_matching[dict_of_OCR[key]].append(dict_of_OCR[i])
                if "Items" in dict_of_OCR[key] and len(dict_of_cols[key]) > 1:
                    label = "Item"
                # elif "Amount" in dict_of_OCR[key] and len(dict_of_cols[key]) > 1:
                #     label = "Price"
                elif "Grand total" in dict_of_OCR[key] and len(dict_of_cols[key]) > 1:
                    label = "Price"
                break
...
if __name__ == "__main__":
    img_paths = glob.glob("/mnt/disk2/viethd/data/FPT/*.jpg")
